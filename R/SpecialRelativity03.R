...
SpaceTimeDiagram <- setRefClass("SpaceTimeDiagram",
                                ...
                                methods = list(
                                  initialize = function (v,x_to,t_to,voy=t_to,x_step=1,t_step=1,x_negative=0) {
                                    ...
                                    stopifnot(length(v) == length(voy)) # v must have the same length as voy(age)
                                    v <<- v
                                    if (x_negative == 0) {
...
# working code #
################
spaceTimeDiagram30k <- SpaceTimeDiagram$new(v = 0.1, x_to = 5, t_to = 10, x_step = 0.5)
spaceTimeDiagram240k <- SpaceTimeDiagram$new(v = 0.8, x_to = 5, t_to = 5, t_step = 0.5)
spaceTimeDiagram240k <- SpaceTimeDiagram$new(v = c(0.8,-0.8), x_to = 5, t_to = 5, voy = c(5,5), t_step = 0.5)
